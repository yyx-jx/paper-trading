...
assert.match(appSource, /api\.getBootstrap\(token, effectiveViewUserId\)/);
...
assert.match(apiSource, /viewedUserId: string/);
assert.match(apiSource, /viewedUser: PublicUser/);
assert.match(apiSource, /getBootstrap\(token: string, viewUserId\?: string\)/);
assert.match(apiSource, /createWsTicket\(token: string, channel: "market" \| "user", viewUserId\?: string\)/);
assert.match(appSource, /className="view-user-control"/);
assert.match(appSource, /className="analytics-view-user-control"/);
assert.match(appSource, /const refreshVisibleViewUsers = useCallback/);
assert.match(appSource, /const handleViewUserChange = \(nextViewUserId: string\) => \{/);
assert.match(appSource, /setViewedUserTarget\(nextViewUserId, nextViewedUser\);/);
assert.match(appSource, /onViewUserChange=\{handleViewUserChange\}/);
assert.match(appSource, /viewUsers=\{visibleViewUsers\}/);
assert.match(appSource, /viewedUserId=\{effectiveViewUserId\}/);
assert.match(appSource, /onUsersChanged=\{refreshVisibleViewUsers\}/);
assert.match(appSource, /const canCreateSingleUser = me\.permissionCodes\.includes\("users:create"\) && \(isAdmin \|\| isGroupManager\)/);
assert.match(appSource, /defaultManagerUserId = managerOptions\.find\(\(user\) => user\.username === "JDH1"\)\?\.id/);
assert.match(appSource, /groupDialog/);
assert.match(appSource, /openGroupDialog/);
assert.match(appSource, /submitGroupChange/);
assert.match(appSource, /api\.changeUserGroup\(token, groupDialog\.user\.id, groupDialog\.managerUserId\)/);
assert.match(apiSource, /changeUserGroup\(token: string, userId: string, managerUserId: string\)/);
assert.match(apiSource, /`\/api\/users\/\$\{userId\}\/group`/);
assert.match(appSource, /localLabel\(language, "换组", "Move group"\)/);
assert.match(appSource, /localLabel\(language, "组别", "Group"\)/);
assert.match(appSource, /\{canCreateSingleUser \? \(/);
assert.match(appSource, /disabled=\{!isAdmin\}/);
assert.match(appSource, /Unassigned \/ Select/);
assert.match(appSource, /canPlaceOrder=\{isViewingSelf && me\.permissionCodes\.includes\("trade:order"\)\}/);
assert.match(appSource, /canManualSettle=\{isViewingSelf && me\.role !== "Tester"\}/);
assert.match(storeSource, /function shouldAcceptViewedPayload/);
assert.match(storeSource, /setViewedUserTarget: \(viewedUserId: string, viewedUser\?: PublicUser\) => void/);
assert.match(storeSource, /setViewedUserTarget: \(viewedUserId, viewedUser\) =>/);
assert.match(storeSource, /profile: undefined,\s*positions: \[\],\s*orders: \[\],\s*orderLifecycles: \[\],\s*logs: \[\]/s);
assert.match(storeSource, /data\.viewedUserId/);
