...
assert.match(i18nSource, /Store accepted age/);
assert.match(i18nSource, /Oldest upstream event age/);
assert.match(i18nSource, /not WebSocket transport latency/);
assert.match(i18nSource, /Snapshot publish gap/);
assert.match(i18nSource, /Server to client/);
assert.match(i18nSource, /Market \{\{p0\}\} \/ User \{\{p1\}\}/);
assert.match(i18nSource, /行情 \{\{p0\}\} \/ 用户 \{\{p1\}\}/);
assert.match(i18nSource, /前端数据年龄/);
assert.match(i18nSource, /\{\{p0\}\} · 上游 \{\{p1\}\}ms \/ 传输 \{\{p2\}\}ms/);
assert.doesNotMatch(i18nSource, /前端落库年龄/);
assert.doesNotMatch(i18nSource, /前端落库 \{\{p0\}\}ms/);
assert.match(i18nSource, /CLOB market stale/);
assert.match(marketSocketSource, /MARKET_SOCKET_TIMING/);
assert.match(readFileSync("apps/client/src/features/market/market-ws-message.ts", "utf8"), /payloadRejectMs: 4000/);
assert.match(readFileSync("apps/client/src/features/market/market-ws-message.ts", "utf8"), /fallbackCooldownMs: 1500/);
