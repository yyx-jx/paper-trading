...
export function buildChainlinkHealthyStatus(input: {
  symbol: string;
  rpcUrl: string;
  ...
  return {
    ...
    sourceEventTs: input.updatedAt,
    serverRecvTs: input.requestFinishTs,
    normalizedTs: input.requestFinishTs,
    serverPublishTs: input.requestFinishTs,
    acquireLatencyMs: Math.max(input.requestFinishTs - input.requestStartTs, 0),
    publishLatencyMs: 0,
    frontendLatencyMs: 0,
    message: `Reading Chainlink AggregatorV3 BTC/USD via ${new URL(input.rpcUrl).host}; Chainlink Data Streams requires paid API credentials.`
  };
}
  ...
  private async poll() {
    ...
    for (let offset = 0; offset < this.clients.length; offset += 1) {
      ...
      try {
        ...
        const requestFinishTs = Date.now();
        this.currentClientIndex = index;
        this.lastSuccessTs = requestFinishTs;
        this.state = {
          price: answer,
          updatedAt,
          status: buildChainlinkHealthyStatus({
